...
const OtrociFor = () => {
  ...
  return (
    <section className="mx-auto container space-y-4 py-6">
      <h1
        ...
        >
          {otrociForTexts.header.split(" ").map((word: string, index: number) => (
            <span
              key={index}
              className={word == "tistega," || word == "those," || word == "otroci" || word == "children" ? "text-primary" : "text-border"}
            >
              {word}{" "}
            </span>
          ))}
      ...
      <br />
      <p className="text-gray-600 space-y-3  px-3 lg:px-[10%]">{otrociForTexts.extra}</p>
    </section>
  );
};
